...
if ((isset($_GET['staffId'])) && ($_GET['staffId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM tbl_staff WHERE staff_id=%s",
                       GetSQLValueString($_GET['staffId'], "int"));

  mysql_select_db($database_millwayconn, $millwayconn);
  $Result1 = mysql_query($deleteSQL, $millwayconn) or die(mysql_error());

  $deleteGoTo = "listStaff.php";
...
if ((isset($_GET['clientID'])) && ($_GET['clientID'] != "")) {
  $deleteSQL = sprintf("DELETE FROM tbl_client WHERE client_ID=%s",
                       GetSQLValueString($_GET['clientID'], "text"));
